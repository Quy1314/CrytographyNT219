# NT219 Windows PowerShell Handbook — Patterned Steps (OpenSSL CLI)
...
# Workspace
$lab = Join-Path $env:USERPROFILE 'nt219-lab'
...
$e   = [int](([regex]::Match(($pub -join ' '), 'Exponent:\s+(\d+)')).Groups[1].Value); if(-not $e){$e=17}
$modLine = & openssl rsa -pubin -in ./rsa_pub.pem -modulus -noout
$modHex  = $modLine -replace '^Modulus=',''
...
# C from kem_raw.c -> BigInteger
$cBytes = [IO.File]::ReadAllBytes((Resolve-Path ./kem_raw.c))
...
# Generate the secret d ()
openssl genpkey -algorithm EC `
  -pkeyopt ec_paramgen_curve:prime256v1 `
  -pkeyopt ec_param_enc:named_curve `
  -out quy_ec.pem
  # Generate the public key d ( Q = dG = G+G+....+G)
openssl pkey -in quy_ec.pem -pubout -out quy_ec_pub.pem

# Verify the keys
openssl ec -in quy_ec.pem -text -noout
openssl pkey -pubin -in quy_ec_pub.pem -text -noout

# An toan ok va chi phi tinh toan thap. 1/7 RSA
